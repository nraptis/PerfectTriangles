...
struct DemoViewRepresentable: UIViewRepresentable {
    ...
    let width: CGFloat
    let height: CGFloat
    func makeUIView(context: UIViewRepresentableContext<DemoViewRepresentable>) -> DemoView {
        let result = DemoView(frame: .zero)
        result.reset(width: width, height: height)
        return result
    }
    
    func updateUIView(_ uiView: DemoView, context: UIViewRepresentableContext<DemoViewRepresentable>) {
        uiView.reset(width: width, height: height)
    }
}
